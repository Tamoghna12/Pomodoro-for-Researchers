...
const ResearchAssistant: React.FC<ResearchAssistantProps> = ({
  ...
  const handleQuickQuery = async () => {
    ...
    try {
      await dispatch(sendQuickQuery(quickQuery.trim()) as any).unwrap()
      setQuickQuery('')
  ...
  return (
    <div className="fixed bottom-4 right-4 w-96 h-[500px] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-40 flex flex-col">
      {/* Header */}
        ...
        <div className="p-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-750">
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-md text-sm"
            >
              <Search className="w-4 h-4" />
            </button>
            <button
              onClick={() => setShowQuickQuery(false)}
              className="px-3 py-2 bg-gray-300 hover:bg-gray-400 dark:bg-gray-600 dark:hover:bg-gray-500 rounded-md text-sm"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
      ...
      {ai.error && (
        <div className="p-3 bg-red-50 dark:bg-red-900/20 border-t border-red-200 dark:border-red-800">
          <div className="flex items-center justify-between">
            <p className="text-xs text-red-600 dark:text-red-400">{ai.error}</p>
            <button
